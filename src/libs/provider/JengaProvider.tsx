...
interface ToastContextType {
  addToast: (
    props: Omit<ToastProps, "id" | "countdown"> & { countdown?: number }
  ) => void;
  toasts: ToastProps[];
}
      ...
      setToasts((currentToasts) =>
        currentToasts
          .map((toast) => ({
            ...toast,
            countdown:
              (toast.countdown ?? 0) > 0 ? (toast.countdown ?? 0) - 1 : 0,
          }))
          .filter((toast) => (toast.countdown ?? 0) > 0)
      );
    }, 1000);
