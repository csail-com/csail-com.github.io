...
export function useMoment(dateParam: string | Date) {
  ...
  const dateString =
    typeof dateParam === "string" ? constructDateString(dateParam) : dateParam;
  ...
  return {
    format: (
      ...
      if (type === "yy년mm월dd일") {
        return `${year
          .toString()
          .slice(-2)}년 ${formattedMonth}월 ${formattedDay}일`;
      }
      ...
      if (type === "yy년mm월dd일 hh:mm") {
        return `${year
          ...
          .slice(
            -2
          )}년 ${formattedMonth}월 ${formattedDay}일 ${period} ${adjustedHours}:${formattedMinutes}`;
      }
    },
