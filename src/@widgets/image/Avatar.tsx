...
const Avatar = ({
  ...
  const source_instance =
    !!source || (!!source && /\.(png|jpg|jpeg|webp)$/.test(source));
  ...
  return (
    <>
      {isLoading ? (
        <Skeleton radius={borderRadius} w={size as number} h={size as number} />
      ) : (
        <>
          {source_instance ? (
            <Image
          ...
          >
            <Image
              ...
              size={{
                width: "100%",
                height: "100%",
                maxWidth: zoomUpImageSize
                  ? zoomUpImageSize
                  : (size as number) * 3.5,
                minWidth: zoomUpImageSize
                  ? zoomUpImageSize
                  : (size as number) * 3.5,
                maxHeight: zoomUpImageSize
                  ? zoomUpImageSize
                  : (size as number) * 3.5,
                minHeight: zoomUpImageSize
                  ? zoomUpImageSize
                  : (size as number) * 3.5,
              }}
              objectFit="cover"
